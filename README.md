...
| `pageLimit` | integer (1-65) | NO | `50` | Number of listings to request per OLX API call. OLX returns at most 65 per page. Increase to 65 to reduce total API requests and run time by ~30%; decrease for finer progress logging. Default 50 is safe across all 6 countries. Only affects request volume -- not which listings are returned. |
